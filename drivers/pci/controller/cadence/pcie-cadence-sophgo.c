...
static int sophgo_pcie_init(struct pci_config_window *cfg)
{
  ...
  phb_dev = dev;
	...
	/*
	 * Retrieve RC base and size from a SG200001 device with _UID
	...
	if (rc->link_id == 0) {
		ret = acpi_get_rc_resources(dev, "SG200001", root->segment, res);
		if (ret){
		  dev_info(dev, "Can't get rc base address\n");
			return -ENOMEM;
		}
